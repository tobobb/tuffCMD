...
part.Touched:connect(function(hit)
	if slap == true then
		if cd == false then
			if not IsDescendantOf(hit, (plr.Character)) then
				if hit.Parent:IsA("Model") then
					...
					if fhum then
						...
						sound:Play()
						local con1
						con1 = game:GetService("RunService").Stepped:connect(function()
							fhum.PlatformStand = true
						end)
						wait(0.1)
						local vel = Instance.new("BodyVelocity",hit)
						vel.velocity = ((hit.Position - plr.Character:WaitForChild("Torso").Position).unit + Vector3.new(0,0.5,0))*50
						...
						htc = hit.Touched:connect(function(hitp)
							if not IsDescendantOf(hitp, hit.Parent) then
								continue = true
								scln6:Stop()
								vel2:Destroy()
								con2:disconnect()
						...
						for i=1,4 do
							...
							p.Color = Color3.new(1,math.random(),0)
							p.CFrame = hit.CFrame
							local con
						...
						repeat wait() until not IsDescendantOf(hit, workspace)
						con2:disconnect()
					end
				end
			end
